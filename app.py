...
JSCAD_SYSTEM = (
    ...
    "primitives.cuboid({ size:[w,d,h] })  // a box\n"
    "primitives.roundedCuboid({ size:[w,d,h], roundRadius:2 })\n"
    "primitives.cylinder({ radius, height, segments:64 })\n"
    "primitives.cylinderElliptic({ height, startRadius:[r,r], endRadius:[0,0], segments:64 })  // cone\n"
    "primitives.sphere({ radius, segments:48 })\n"
    "primitives.torus({ innerRadius, outerRadius, innerSegments:32, outerSegments:64 })  // ring/donut\n"
    "booleans.union(a, b, ...)   booleans.subtract(a, hole, ...)   booleans.intersect(a, b)\n"
    "transforms.translate([x,y,z], geom)   transforms.rotate([rx,ry,rz], geom)   transforms.scale([sx,sy,sz], geom)\n"
    "extrusions.extrudeLinear({ height }, shape2d)\n\n"
    "EXACT API — getting names wrong will crash it:\n"
    "- Destructure ONLY these: const { primitives, booleans, transforms, extrusions, hulls } = jscad\n"
    "- Boolean ops live under `booleans` and take SEPARATE arguments, NOT an array. "
    "There is NO `unions`/`union`/`subtract` at the top level. To merge a LIST use spread: "
    "booleans.union(...myArray). Correct: booleans.union(a, b, c); booleans.subtract(body, hole).\n"
    "- Every shape is primitives.X, every move/rotate is transforms.X.\n\n"
    "RULES:\n"
